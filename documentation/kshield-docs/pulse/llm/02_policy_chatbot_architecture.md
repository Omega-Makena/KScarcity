...
**Status**: Implemented (active in SENTINEL)
...
### 1.1 Implemented Additions (Current)

- News ingestion now enriches articles with full-content extraction (`extracted_text`) and trace metadata.
- Chat outputs now include mandatory evidence trace lines:
  - source URL(s),
  - short evidence excerpt(s),
  - stored pointer(s): article/content record IDs and storage path where available.
- Document Intelligence surfaces evidence excerpts and trace pointers for news cards.
