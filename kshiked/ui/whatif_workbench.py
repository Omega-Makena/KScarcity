...
def _render_causal_diagnosis(data, theme):
  """Causal evidence panel surfaced after a simulation run.

  Shows a compact ATE card for the most policy-relevant treatment→outcome pair
  found in the loaded World Bank dataset. Hidden inside an expander so it
  does not crowd the main results view.
  """
  try:
  ...
  with st.expander("Causal Evidence — what's actually driving this scenario?", expanded=False):
    st.caption(
      "Uses Scarcity's structural causal engine to estimate the average effect (ATE) "
      "of the primary shock driver on the outcome you care about most. "
      "This is separate from the simulation trajectory above — it's a data-driven "
      "causal estimate from historical observations, not a forward projection."
    )

    df = load_world_bank_data()
    if df.empty:
      st.info("Load a World Bank dataset (in the Causal tab) to see causal evidence here.")
      return

    scenario = st.session_state.get("_whatif_scenario_obj")
    # Pick the most relevant treatment / outcome from the scenario if available,
    # else let the user choose from available columns.
    avail = [c for c in df.columns if df[c].notna().sum() >= 15]
    default_treatments = [
      c for c in avail
      if any(kw in c.lower() for kw in ("inflation", "tax", "fuel", "interest", "gdp"))
    ]
    default_outcomes = [
      c for c in avail
      if any(kw in c.lower() for kw in ("unemployment", "growth", "poverty", "gini"))
    ]
    ...
    with col_t:
      treatment = st.selectbox(
        "Cause (treatment)",
        avail,
        index=avail.index(default_treatments[0]) if default_treatments else 0,
        key="cdiag_treatment",
      )
    with col_o:
      outcome_opts = [c for c in avail if c != treatment]
      default_out_idx = (
        outcome_opts.index(default_outcomes[0])
        if default_outcomes and default_outcomes[0] in outcome_opts
        else 0
      )
...
def _render_results(data, theme, outcome_dimensions, default_dimensions,
  ...
  # Causal diagnosis — tucked in expander, non-intrusive
  _render_causal_diagnosis(data, theme)
