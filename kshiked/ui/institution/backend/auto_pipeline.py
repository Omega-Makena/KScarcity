...
class AutoPipeline:
  ...
  def _build_narrative(cls, r: PipelineResult) -> str:
    ...
    # Top relationship
    if r.relationships:
      _top_rel = sorted(r.relationships, key=lambda x: float(x.get("confidence", 0.0)), reverse=True)[0]
      _vars = _top_rel.get("variables", [])
      _conf = float(_top_rel.get("confidence", 0.0))
      _rtype = str(_top_rel.get("rel_type", "related_to")).replace("_", " ")
      if len(_vars) >= 2:
        if _conf >= 0.6:
          parts.append(
            f"Top discovered relationship: **{_vars[0]}** {_rtype} **{_vars[1]}** "
            f"(confidence: {_conf:.0%})."
          )
        else:
          parts.append(
            f"Top detected association (still tentative): **{_vars[0]}** {_rtype} **{_vars[1]}** "
            f"(confidence: {_conf:.0%})."
          )
    elif r.relationship_summary:
      parts.append(f"Top discovered relationship: {r.relationship_summary[0]}.")
    ...
    if r.forecast_matrix:
      _horizon = len(r.forecast_matrix)
      _avg_var = 0.0
      _var_count = 0
      for _row in r.variance_matrix or []:
        for _v in _row:
          try:
            _avg_var += abs(float(_v))
            _var_count += 1
          except Exception:
            continue
      _avg_var = (_avg_var / _var_count) if _var_count else 0.0
      _uncertainty = "low" if _avg_var < 0.25 else "moderate" if _avg_var < 1.0 else "elevated"
      parts.append(
        f"A {_horizon}-step trajectory forecast was generated. "
        f"Model uncertainty is currently {_uncertainty}."
      )
