...
def _render_structural_inference(df: pd.DataFrame, cols: List[str], theme):
  ...
  st.markdown("**Choose Effect Types (Estimands) â€” Plain-English Labels**")
  estimand_help = [
    ("ATE", "Average effect for everyone"),
    ("ATT", "Average effect for treated group"),
    ("ATC", "Average effect for untreated/control group"),
  ...
  estimand_labels = [f"{code} â€” {desc}" for code, desc in estimand_help]
  default_codes = ["ATE", "ATT", "ATC"]
  if HAS_ECONML and effect_modifiers:
    default_codes.append("CATE")
  default_labels = [label for label in estimand_labels if label.split(" â€” ", 1)[0] in default_codes]
  ...
  selected_estimands = [label.split(" â€” ", 1)[0] for label in selected_labels]
...
def render_causal_evidence_panel(
  ...
  try:
    ...
    st.caption("Causal intelligence unavailable: scarcity causal engine not found.")
    return
  ...
  ate_val = _to_scalar(ate_artifact.estimate)
