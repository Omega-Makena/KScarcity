"""
K-SHIELD: Causal Relationships â€” Auto-Discovery Engine
...
# â”€â”€ Heavy ML/stats packages loaded LAZILY on first use â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# Compatibility shims â€” check availability without importing
@property
...
HAS_STATSMODELS = None # sentinel â€” replaced by _check functions
HAS_DOWHY = None
HAS_SKLEARN = None
HAS_ECONML = None
HAS_NETWORKX = None
...
# â”€â”€ Resolve lazy flags on first real access â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _init_lazy_flags():
  """Populate HAS_* globals lazily â€” call before any HAS_* check."""
...
# DATA LOADING â€” AUTO-DISCOVERY (no hardcoded indicators)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def _validate_and_load_upload(uploaded_file) -> Tuple[Optional[pd.DataFrame], Optional[str]]:
  ...
  # Check 2: Simple format â€” first column is time index, rest are numeric series
  ...
  with content_col:
    if section == "1) Why this page exists":
      ...
      st.markdown(
        ...
        *Do past values of X help predict future values of Y, beyond Yâ€™s own past?*
...
def render_causal(theme, data=None):
  ...
  # â”€â”€ Data source selector â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  st.markdown("---")
  source = st.radio(
    "Data Source",
  ...
  # â”€â”€ Indicator selection (auto-discovered) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  st.markdown("---")
  available = sorted(df.columns.tolist())

  # No hardcoded defaults â€” pick the first few alphabetically
...
def _render_upload_section(theme):
  ...
  st.markdown(
    ...
    f'margin-bottom:0.5rem; font-family:IBM Plex Mono,monospace; '
...
def _render_scatter_matrix(df: pd.DataFrame, cols: List[str], theme):
  ...
  fig.update_layout(**_base_layout(
    ...
    font=dict(
      family="IBM Plex Sans, sans-serif", color=theme.text_muted, size=8
    ),
  ))
...
def _render_structural_inference(df: pd.DataFrame, cols: List[str], theme):
  ...
  # â”€â”€ Contextual estimand picker â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  st.markdown("**What do you want to measure?**")
  st.caption(
    "Only relevant questions are shown. Fill in the optional fields above "
    "(Effect Modifiers, Instrument, Mediator) to unlock additional analyses."
  )

  selected_estimands: List[str] = []

  # Tier 1 â€” always available
  ate_on = st.checkbox(
    "How much? â€” average effect across everyone (ATE)",
    value=True,
    key="est_ate",
    help="The average causal effect of the treatment on the outcome across the whole dataset.",
  )
  if ate_on:
    selected_estimands.append("ATE")

  # Tier 2 â€” contextually unlocked by the fields filled above
  _any_tier2 = bool(effect_modifiers) or bool(instrument) or bool(mediator)
  if _any_tier2:
    st.markdown(
      f'<div style="color:{theme.text_muted}; font-size:0.78rem; '
      f'margin-top:0.4rem; margin-bottom:0.2rem;">'
      f'Additional questions unlocked by your selections above:</div>',
      unsafe_allow_html=True,
    )

  if effect_modifiers and HAS_ECONML:
    if st.checkbox(
      "Who is affected differently? â€” effect by subgroup/county (CATE)",
      value=True,
      key="est_cate",
      help="Estimates heterogeneous effects across subgroups defined by your effect modifiers.",
    ):
      selected_estimands.append("CATE")
  elif effect_modifiers and not HAS_ECONML:
    st.caption(" 'Who is affected differently?' requires **econml** â€” `pip install econml`")

  if instrument:
    if st.checkbox(
      "Natural experiment effect â€” uses instrument as exogenous shock (LATE)",
      value=True,
      key="est_late",
      help="Uses the instrument as a natural experiment to estimate an unconfounded causal effect.",
    ):
      selected_estimands.append("LATE")

  if mediator:
    if st.checkbox(
      "Direct pathway â€” effect NOT flowing through the mediator (NDE)",
      value=True,
      key="est_nde",
      help="How much of the treatment effect reaches the outcome directly, bypassing the mediator?",
    ):
      selected_estimands.append("MEDIATION_NDE")
    if st.checkbox(
      "Indirect pathway â€” effect channelled through the mediator (NIE)",
      value=False,
      key="est_nie",
      help="How much of the treatment effect is mediated? (e.g. sanctions â†’ unemployment â†’ instability)",
    ):
      selected_estimands.append("MEDIATION_NIE")

  # Tier 3 â€” expert options in expander
  with st.expander("More effect types (expert)", expanded=False):
    st.caption("Useful for specific research questions; not needed for most analyses.")
    if st.checkbox(
      "Effect on treated group only (ATT)",
      value=False,
      key="est_att",
      help="Average effect on units that actually received the treatment.",
    ):
      selected_estimands.append("ATT")
    if st.checkbox(
      "Effect on untreated group only (ATC)",
      value=False,
      key="est_atc",
      help="Counterfactual: what if the untreated units had been treated?",
    ):
      selected_estimands.append("ATC")
    if HAS_ECONML and st.checkbox(
      "Individual-level effect (ITE)",
      value=False,
      key="est_ite",
      help="Per-row treatment effect estimate via EconML CausalForest.",
    ):
      selected_estimands.append("ITE")

  if not selected_estimands:
    st.warning("Enable at least one effect type above.")
    return
  st.caption("Running: " + " Â· ".join(selected_estimands))
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# REUSABLE CAUSAL EVIDENCE PANEL â€” embeddable in other tabs
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

def render_causal_evidence_panel(
  df,
  treatment: str,
  outcome: str,
  ...
  """
  Compact causal evidence card for embedding in non-causal tabs.

  Always runs ATE (the headline number). When *group_col* is supplied and
  econml is available, also runs CATE and shows a per-group bar chart inside
  an expander â€” the user only sees the ATE card by default.

  Args:
    df: Time-indexed DataFrame containing the columns.
    treatment: Column name to treat as the cause.
    outcome: Column name to treat as the effect.
    confounders: Columns to control for. Auto-detected if None.
    group_col: Optional column for subgroup breakdown (CATE).
    theme: SENTINEL theme object for colours.
  ...
  if treatment not in df.columns or outcome not in df.columns:
    st.caption(f"Causal evidence unavailable: {treatment} or {outcome} not in dataset.")
    return
  if not HAS_DOWHY or not HAS_SKLEARN:
    st.caption("Causal evidence requires **dowhy** and **scikit-learn**.")
    return

  try:
    from scarcity.causal.engine import run_causal
    from scarcity.causal.specs import EstimandSpec, EstimandType, RuntimeSpec, ParallelismMode
  except Exception:
    return

  # Auto-detect confounders if not supplied
  if confounders is None:
    confounders = [
      c for c in df.columns
      if c not in (treatment, outcome, group_col)
      and df[c].notna().sum() >= 10
    ][:4] # cap at 4 to keep it fast

  clean = df[[treatment, outcome] + confounders].dropna()
  if len(clean) < 15:
    st.caption("Causal evidence unavailable: not enough complete observations.")
    return
  ...
  run_cate = group_col and group_col in df.columns and HAS_ECONML
  if run_cate:
    specs.append(
      EstimandSpec(
        treatment=treatment,
  ...
  with st.spinner(f"Estimating causal effect of {treatment} â†’ {outcome} â€¦"):
    try:
      result = run_causal(clean, specs, runtime)
    except Exception as exc:
      st.caption(f"Causal evidence failed: {exc}")
      return

  if not result.results:
    st.caption("Causal estimation returned no results.")
    return

  ate_artifact = next(
    (a for a in result.results if a.estimand_type == "ATE"), result.results[0]
  )
  ate_val = _to_scalar_estimate(ate_artifact.estimate)

  # Headline card
  direction = "increases" if ate_val > 0 else ("decreases" if ate_val < 0 else "does not shift")
  sign_color = (
    (theme.accent_danger if ate_val > 0 else theme.accent_success)
    if theme else ("#ef4444" if ate_val > 0 else "#22c55e")
  )
  muted = theme.text_muted if theme else "#888"
  card_bg = theme.bg_card if theme else "rgba(255,255,255,0.05)"
  border = theme.border_default if theme else "rgba(255,255,255,0.1)"

  st.markdown(
    f'<div style="background:{card_bg}; border:1px solid {border}; '
    f'border-radius:8px; padding:0.8rem 1rem; display:flex; '
    f'align-items:center; gap:1.2rem;">'
    f'<div style="font-size:0.75rem; color:{muted}; line-height:1.4;">'
    f'<b>{treatment}</b> causally <b>{direction}</b> <b>{outcome}</b><br>'
    f'<span style="font-size:0.7rem;">ATE Â· {len(clean)} observations Â· linear regression</span>'
    f'</div>'
    f'<div style="font-size:1.4rem; font-weight:700; color:{sign_color}; '
    f'font-family:IBM Plex Mono,monospace; white-space:nowrap;">'
    f'{"+" if ate_val > 0 else ""}{ate_val:.3f}'
    f'</div>'
    f'</div>',
    unsafe_allow_html=True,
  )

  # Optional CATE breakdown â€” hidden in expander so it doesn't crowd the parent tab
  cate_artifact = next(
    (a for a in result.results if a.estimand_type == "CATE"), None
  ) if run_cate else None

  if cate_artifact is not None and HAS_PLOTLY:
    import plotly.graph_objects as go
    cate_val = _to_scalar_estimate(cate_artifact.estimate)
    with st.expander(f"Subgroup breakdown by {group_col}", expanded=False):
      st.caption(
        f"CATE estimate: {cate_val:+.3f}. "
        "A full per-group chart requires running the dedicated Causal Analysis tab "
        "with effect modifiers set."
      )

  if result.errors:
    with st.expander("Estimation warnings", expanded=False):
      for err in result.errors:
        st.caption(f"{err.spec_id}: {err.message}")
...
def _build_graph_from_granger(df: pd.DataFrame, cols: List[str]) -> str:
  ...
  dot.append(' node [fontname="IBM Plex Sans", shape=box, style=filled, '
