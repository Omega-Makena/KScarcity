...
def render_io_sectors_tab(theme):
  ...
  if flow_vals:
    hex_to_rgba = lambda h, a: f"rgba({int(h[1:3], 16)},{int(h[3:5], 16)},{int(h[5:7], 16)},{a})" if h.startswith("#") else h
    fig_sankey = go.Figure(go.Sankey(
      ...
      link=dict(source=src_nodes, target=tgt_nodes, value=flow_vals,
           label=flow_labels,
           color=[hex_to_rgba(PALETTE[s % len(PALETTE)], 0.3) for s in src_nodes]),
    ))
...
def render_research_engine_tab(theme, SFCEconomy, SFCConfig, calibrate_from_data):
  ...
  gov_spending = gdp * research_econ.economy.config.spending_ratio
